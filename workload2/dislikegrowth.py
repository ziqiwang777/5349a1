...
if __name__ == "__main__":
    ...
    moviedata = rawdata.map(extractdata1) #vedio_id,vedio_country,trending_date,likes,dislikes
    # groupdata = moviedata.groupByKey().filter(lambda l : l[0] != None).filter(lambda l : len(l[1])>1)

    # to decide which category the vedio belong to by compare the trending_date
    category_vidiodata = rawdata.map(extractdata2) #vedio_id category trending_date
